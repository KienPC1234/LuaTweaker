...
-- CONFIG-DRIVEN TUNING (luaconfig/ruby_mod.json ΓÇö default_config.json on first launch)
...
local function handleStaffUse(player, itemStack)
    ...
    if skillIdx == 1 then
        -- SKILL 1: RUBY ORB FIREBALL ΓÇö turns homing when a target is marked.
    ...
    elseif skillIdx == 4 then
        -- SKILL 4: HOMING RUBY DART ΓÇö auto-locks the nearest visible entity,
...
if Network then
    ...
    -- KEYBIND 'X': MARK TARGET ΓÇö raycast the entity the player is looking at,
