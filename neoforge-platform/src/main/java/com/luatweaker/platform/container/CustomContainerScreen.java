...
/**
 * Client screen for Lua-configured container blocks. The panel texture is a plain
 * 176x190 frame with the vanilla-style player inventory; the container's own slot
...
public class CustomContainerScreen extends AbstractContainerScreen<CustomContainerMenu> {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        int cols = menu.getContainerCols();
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                int sx = x + 34 + col * 18;
                int sy = y + 17 + row * 18;
                guiGraphics.blit(SLOT_TEXTURE, sx - 1, sy - 1, 0, 0, 18, 18, 18, 18);
            }
        }
    }
